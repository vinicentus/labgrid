...
def create_place(coordinator):
    ...
    def _create_place(place_name):
        with pexpect.spawn(f'{sys.executable} -m labgrid.remote.client -p {place_name} create') as spawn:
            spawn.expect(pexpect.EOF)
        assert spawn.exitstatus == 0, spawn.before.strip()

        place_names.append(place_name)

        with pexpect.spawn(f'{sys.executable} -m labgrid.remote.client -p {place_name} set-tags board=123board') as spawn:
            spawn.expect(pexpect.EOF)
        assert spawn.exitstatus == 0, spawn.before.strip()
    ...
    for place_name in place_names:
        # clean up
        with pexpect.spawn(f'{sys.executable} -m labgrid.remote.client -p {place_name} release') as spawn:
            spawn.expect(pexpect.EOF)

        with pexpect.spawn(f'{sys.executable} -m labgrid.remote.client -p {place_name} delete') as spawn:
            spawn.expect(pexpect.EOF)
        assert spawn.exitstatus == 0, spawn.before.strip()

@pytest.fixture(scope='function')
def place_acquire(place, exporter):
    with pexpect.spawn(f'{sys.executable} -m labgrid.remote.client -p test add-match "*/Testport/*"') as spawn:
        spawn.expect(pexpect.EOF)
        spawn.close()
        assert spawn.exitstatus == 0, spawn.before.strip()

    with pexpect.spawn(f'{sys.executable} -m labgrid.remote.client -p test acquire') as spawn:
        spawn.expect(pexpect.EOF)
        spawn.close()
        assert spawn.exitstatus == 0, spawn.before.strip()
    ...
    with pexpect.spawn(f'{sys.executable} -m labgrid.remote.client -p test release') as spawn:
        spawn.expect(pexpect.EOF)
        spawn.close()
        assert spawn.exitstatus == 0, spawn.before.strip()

def test_connect_error():
    with pexpect.spawn(f'{sys.executable} -m labgrid.remote.client -x 127.0.0.1:20409 places') as spawn:
...
def test_connect_timeout(coordinator):
    ...
    try:
        with pexpect.spawn(f'{sys.executable} -m labgrid.remote.client places') as spawn:
...
def test_place_show(place):
    with pexpect.spawn(f'{sys.executable} -m labgrid.remote.client -p test show') as spawn:
...
def test_place_alias(place):
    with pexpect.spawn(f'{sys.executable} -m labgrid.remote.client -p test add-alias foo') as spawn:
        spawn.expect(pexpect.EOF)
        spawn.close()
        assert spawn.exitstatus == 0, spawn.before.strip()

    with pexpect.spawn(f'{sys.executable} -m labgrid.remote.client -p foo del-alias foo') as spawn:
        spawn.expect(pexpect.EOF)
        spawn.close()
        assert spawn.exitstatus == 0, spawn.before.strip()

def test_place_comment(place):
    with pexpect.spawn(f'{sys.executable} -m labgrid.remote.client -p test set-comment my comment') as spawn:
        spawn.expect(pexpect.EOF)
        spawn.close()
        assert spawn.exitstatus == 0, spawn.before.strip()
...
def test_place_match(place):
    with pexpect.spawn(f'{sys.executable} -m labgrid.remote.client -p test add-match "e1/g1/r1" "e2/g2/*"') as spawn:
        spawn.expect(pexpect.EOF)
        spawn.close()
        assert spawn.exitstatus == 0, spawn.before.strip()

    with pexpect.spawn(f'{sys.executable} -m labgrid.remote.client -p test del-match "e1/g1/r1"') as spawn:
        spawn.expect(pexpect.EOF)
        spawn.close()
        assert spawn.exitstatus == 0, spawn.before.strip()
...
def test_place_match_duplicates(place):
    ...
    for match in matches:
        with pexpect.spawn(f'{sys.executable} -m labgrid.remote.client -p test add-match "{match[0]}" "{match[1]}"') as spawn:
            spawn.expect(f"pattern '{match[1]}' exists, skipping")
            spawn.expect(pexpect.EOF)
            spawn.close()
            assert spawn.exitstatus == 0, spawn.before.strip()

        with pexpect.spawn(f'{sys.executable} -m labgrid.remote.client -p test del-match "{match[0]}"') as spawn:
            spawn.expect(pexpect.EOF)
            spawn.close()
            assert spawn.exitstatus == 0, spawn.before.strip()
...
def test_place_acquire(place):
    ...
    with pexpect.spawn(f'{sys.executable} -m labgrid.remote.client who') as spawn:
        spawn.expect(".*test.*")
        spawn.expect(pexpect.EOF)
        spawn.close()
        assert spawn.exitstatus == 0, spawn.before.strip()
...
def test_place_acquire_multiple(create_place, tmpdir):
    ...
    with pexpect.spawn(f'{sys.executable} -m labgrid.remote.client -c {p} acquire') as spawn:
        spawn.expect(pexpect.EOF)
    assert spawn.exitstatus == 0, spawn.before.strip()
    ...
    with pexpect.spawn(f'{sys.executable} -m labgrid.remote.client -c {p} release') as spawn:
        spawn.expect(pexpect.EOF)
    assert spawn.exitstatus == 0, spawn.before.strip()
...
def test_place_acquire_enforce(place):
    with pexpect.spawn(f'{sys.executable} -m labgrid.remote.client -p test add-match does/not/exist') as spawn:
        spawn.expect(pexpect.EOF)
        spawn.close()
        assert spawn.exitstatus == 0, spawn.before.strip()
    ...
    with pexpect.spawn(f'{sys.executable} -m labgrid.remote.client -p test acquire --allow-unmatched') as spawn:
...
def test_place_acquire_broken(place, exporter):
    with pexpect.spawn(f'{sys.executable} -m labgrid.remote.client -p test add-match "*/Broken/*"') as spawn:
        spawn.expect(pexpect.EOF)
        spawn.close()
        assert spawn.exitstatus == 0, spawn.before.strip()
...
def test_place_release_from(monkeypatch, place, exporter):
    ...
    with pexpect.spawn(f'{sys.executable} -m labgrid.remote.client -p test release-from foo/bar') as spawn:
        spawn.expect(pexpect.EOF)
        spawn.close()
        assert spawn.exitstatus == 0, spawn.before.strip()
    ...
    with pexpect.spawn(f'{sys.executable} -m labgrid.remote.client -p test release-from {host}/{user}') as spawn:
        spawn.expect(pexpect.EOF)
        spawn.close()
        assert spawn.exitstatus == 0, spawn.before.strip()
...
def test_place_add_no_name(coordinator):
    with pexpect.spawn(f'{sys.executable} -m labgrid.remote.client create') as spawn:
...
def test_place_del_no_name(coordinator):
    with pexpect.spawn(f'{sys.executable} -m labgrid.remote.client delete') as spawn:
...
def test_resource_conflict(place_acquire, tmpdir):
    with pexpect.spawn(f'{sys.executable} -m labgrid.remote.client -p test2 create') as spawn:
        spawn.expect(pexpect.EOF)
        spawn.close()
        assert spawn.exitstatus == 0, spawn.before.strip()

    with pexpect.spawn(f'{sys.executable} -m labgrid.remote.client -p test2 add-match "*/Testport/*"') as spawn:
        spawn.expect(pexpect.EOF)
        spawn.close()
        assert spawn.exitstatus == 0, spawn.before.strip()

    with pexpect.spawn(f'{sys.executable} -m labgrid.remote.client -p test2 acquire') as spawn:
        spawn.expect(pexpect.EOF)
        spawn.close()
        assert spawn.exitstatus != 0, spawn.before.strip()

    with pexpect.spawn(f'{sys.executable} -m labgrid.remote.client -p test2 delete') as spawn:
        spawn.expect(pexpect.EOF)
        spawn.close()
        assert spawn.exitstatus == 0, spawn.before.strip()

def test_reservation(place_acquire, tmpdir):
    with pexpect.spawn(f'{sys.executable} -m labgrid.remote.client reserve --shell board=123board name=test') as spawn:
        spawn.expect(pexpect.EOF)
        spawn.close()
        assert spawn.exitstatus == 0, spawn.before.strip()
    ...
    with pexpect.spawn(f'{sys.executable} -m labgrid.remote.client reservations') as spawn:
        spawn.expect(pexpect.EOF)
        spawn.close()
        assert spawn.exitstatus == 0, spawn.before.strip()
    ...
    with pexpect.spawn(f'{sys.executable} -m labgrid.remote.client -p + acquire', env=env) as spawn:
        spawn.expect(pexpect.EOF)
        spawn.close()
        assert spawn.exitstatus == 0, spawn.before.strip()

    with pexpect.spawn(f'{sys.executable} -m labgrid.remote.client -p + show', env=env) as spawn:
        spawn.expect(pexpect.EOF)
        spawn.close()
        assert token in spawn.before, spawn.before.strip()
        assert spawn.exitstatus == 0, spawn.before.strip()

    with pexpect.spawn(f'{sys.executable} -m labgrid.remote.client -p + release', env=env) as spawn:
        spawn.expect(pexpect.EOF)
        spawn.close()
        assert spawn.exitstatus == 0, spawn.before.strip()
    ...
    with pexpect.spawn(f'{sys.executable} -m labgrid.remote.client cancel-reservation', env=env) as spawn:
        spawn.expect(pexpect.EOF)
        spawn.close()
        assert spawn.exitstatus == 0, spawn.before.strip()
...
def test_resource_acquired_state_on_exporter_restart(monkeypatch, place, exporter):
    ...
    # add resource match
    with pexpect.spawn(f'{sys.executable} -m labgrid.remote.client -p test add-match testhost/Testport/NetworkSerialPort') as spawn:
        spawn.expect(pexpect.EOF)
        spawn.close()
        assert spawn.exitstatus == 0, spawn.before.strip()
    ...
    with pexpect.spawn(f'{sys.executable} -m labgrid.remote.client -p test -v resources') as spawn:
        spawn.expect(pexpect.EOF)
        spawn.close()
        assert spawn.exitstatus == 0, spawn.before.strip()
...
def test_exporter_timeout(place, exporter):
    with pexpect.spawn(f'{sys.executable} -m labgrid.remote.client resources') as spawn:
        spawn.expect(pexpect.EOF)
        spawn.close()
        assert spawn.exitstatus == 0, spawn.before.strip()
...
def test_reservation_custom_config(place, exporter, tmpdir):
    ...
    with pexpect.spawn(f'{sys.executable} -m labgrid.remote.client -c {p} reserve --wait --shell board=123board name=test') as spawn:
        spawn.expect(pexpect.EOF)
        spawn.close()
        assert spawn.exitstatus == 0, spawn.before.strip()
        m = re.search(rb"^export LG_TOKEN=(\S+)$", spawn.before.replace(b'\r\n', b'\n'), re.MULTILINE)
    ...
    with pexpect.spawn(f'{sys.executable} -m labgrid.remote.client -c {p} -p + lock', env=env) as spawn:
        spawn.expect("acquired place test")
        spawn.expect(pexpect.EOF)
        spawn.close()
        assert spawn.exitstatus == 0, spawn.before.strip()

    with pexpect.spawn(f'{sys.executable} -m labgrid.remote.client -c {p} -p + release', env=env) as spawn:
...
def test_same_name_resources(place, exporter, tmpdir):
    with pexpect.spawn(f'{sys.executable} -m labgrid.remote.client -p test add-named-match "testhost/Many/NetworkService" "samename"') as spawn:
        spawn.expect(pexpect.EOF)
        spawn.close()
        assert spawn.exitstatus == 0, spawn.before.strip()

    with pexpect.spawn(f'{sys.executable} -m labgrid.remote.client -p test add-named-match "testhost/Many/NetworkSerialPort" "samename"') as spawn:
        spawn.expect(pexpect.EOF)
        spawn.close()
        assert spawn.exitstatus == 0, spawn.before.strip()
    ...
    with pexpect.spawn(f'{sys.executable} -m labgrid.remote.client -p test env') as spawn:
        spawn.expect(pexpect.EOF)
        spawn.close()
        assert spawn.exitstatus == 0, spawn.before.strip()
